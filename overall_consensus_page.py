...
# 🔥 [ULTIMATE ULTRACLEAN ENGINE] ஸ்ட்ரிங், இன்டஜர், எமோஜி என அனைத்தையும் சரிசெய்யும் ஃபங்க்ஷன்
def get_clean_rank(df, id_col, sup_id, algo_name):
    # ஐடியை ஸ்ட்ரிங் மற்றும் இன்டஜர் இரண்டு வகையிலும் தேட ஏதுவாக மாற்றுகிறோம்
    df_temp = df.copy()
    df_temp[id_col] = df_temp[id_col].astype(str).str.strip()
    target_id_str = str(sup_id).strip()
    
    sub_df = df_temp[df_temp[id_col] == target_id_str]
    if sub_df.empty:
        return 1
    ...
    # 2. ஒருவேளை ரேங்க் காலம் மேட்ச் ஆகவில்லை என்றால் ஸ்கோர் காலம்களை வச்சு இங்கேயே ரேங்க் போடுகிறது!
    score_cols = [c for c in df_temp.columns if any(w in c.lower() for w in ['score', 'value', 'performance', 'assessment', 'q_', 'v_']) or c not in [id_col]]
    if score_cols:
        ...
        return int(df_temp.loc[df_temp[id_col] == target_id_str, 'Calculated_Rank'].values[0])
...
if not has_active_data:
    ...
    for sup in all_suppliers:
        ...
        # கச்சிதமான காலம் மேப்பிங் மற்றும் ரேங்க் எக்ஸ்ட்ராக்ஷன்
...
if df_ranks is not None:
    ...
    # இருக்கும் காலம்களை மட்டும் எடுத்துக்கொள்கிறது (பாதுகாப்பிற்காக)
    ...
    # நீங்கள் கேட்ட வரிசைப்படியே காலம்களை ரீ-ஆர்டர் (Reorder) செய்கிறோம்
